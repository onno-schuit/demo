...
class core_message_renderer extends plugin_renderer_base {
    ...
    public function manage_messagingoptions($processors, $providers, $preferences, $defaultpreferences, $notificationsdisabled = false) {
        ...
        // Added by Onno Schuit for THNK Link project
        $thnk_link_providers = array('instantmessage', 'posts');
        // Populate the table with rows
        foreach ( $providers as $provider) {
            // Added by Onno Schuit for THNK Link project
            if (! in_array($provider->name, $thnk_link_providers) ) continue;
        ...
        $output .= "<a name='generalsettings'>&nbsp;</a>";

        $output .= html_writer::start_tag('div');
        $output .= get_string('icanonlybecontactedby') . ': '; 
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_tag('div');
        $output .= html_writer::checkbox('membersofmygroups', 1, $preferences->membersofmygroups, '');
        $output .= ' ' . get_string('membersofmygroups'); 
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_tag('div');
        $output .= html_writer::checkbox('blocknoncontacts', 1, $preferences->blocknoncontacts, '');
        $output .= ' ' . get_string('peoplelistedascontacts'); 
        $output .= html_writer::end_tag('div');

        $excluded_roles = array('frontpage', 'guest', 'user');
        $roles = get_all_roles();
        $renamed_roles = role_fix_names($roles, get_context_instance(CONTEXT_SYSTEM));
        $localized_roles = array('' => '[' .get_string('role') . ']');
        foreach($renamed_roles as $role) {
            if (in_array($role->shortname, $excluded_roles)) continue;
            $localized_roles[$role->shortname] = $role->localname;
        }

        $output .= html_writer::start_tag('div');
        $output .= get_string('userswiththeseroles') . ': ';
        $output .= html_writer::select($localized_roles, 'userswiththeseroles', $preferences->userswiththeseroles, false);
        $output .= html_writer::end_tag('div');
        ...
        $output .= html_writer::end_tag('form');
        return $output;
    } // function manage_messagingoptions
