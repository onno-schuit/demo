...
if ($currentuser && !empty($user2) && has_capability('moodle/site:sendmessage', $context)) {
    ...
    if (empty($messageerror) && message_is_blocking($user2->id, $USER->id, $userpreferences) ) {
        $messageerror = get_string('userisblockingyounoncontact', 'message');
    }
...
if ($cm_id = message_get_connector_cm_id($COURSE->id)) {
    echo "<div><a id='message_linktoconnector' href='{$CFG->wwwroot}/mod/connector/index.php?id=$cm_id&controller=user'>" . get_string('gotoconnector', 'connector') . "</a></div>";
}
