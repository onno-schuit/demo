...
function message_get_contacts($user1=null, $user2=null) {
    ...
    $contactsql = "SELECT $userfields, COUNT(m.id) AS messagecount
                    ...
                    WHERE mc.userid = ? AND mc.blocked = 0 AND u.deleted = 0
                 GROUP BY $userfields
                 ORDER BY u.firstname ASC";
...
function message_print_usergroup_selector($viewing, $courses, $coursecontexts, $countunreadtotal, $countblocked, $strunreadmessages) {
    ...
    /*
    // Removed at client's request by Onno Schuit
...
/**
 * Check if recipient is blocking sender.
 * Added by Onno Schuit for Thnk Lnk project, 2012-01-30.
 *
 * @param  integer   $recipient_id     User id of the recipient
 * @param  integer   $sender_id        User id of the sender
 * @param  array     $user_preferences Preferences of user, defaults to false (optional)
 * @return boolean                     Returns true if recipient is blocking the sender
 */
function message_is_blocking($recipient_id, $sender_id, $user_preferences = false) {
    $user_preferences = ($user_preferences) ? $user_preferences : get_user_preferences(NULL, NULL, $recipient_id);

    if (!empty($user_preferences['message_membersofmygroups'])) {
        if (!message_is_fellow_member_of($recipient_id, $sender_id)) return true;
    }

    if (!empty($user_preferences['message_userswiththeseroles'])) {
        if (!message_has_role($sender_id, $user_preferences['message_userswiththeseroles'])) return true;
    }
    return false;
} // function message_is_blocking


// Returns true if repicient and sender share a group membership
function message_is_fellow_member_of($recipient_id, $sender_id) {
    global $DB;
    $sql = "SELECT COUNT(g1.groupid)
            FROM {groups_members} AS g1
            WHERE g1.userid = :recipient_id
            AND EXISTS (
                SELECT *
                FROM {groups_members}
                WHERE userid = :sender_id
                AND groupid = g1.groupid
            )";
    $count = $DB->get_field_sql($sql, array('sender_id' => $sender_id, 'recipient_id' => $recipient_id));
    return ($count > 0);           
} // function message_is_fellow_member_of


function message_has_role($sender_id, $role) {
    global $DB, $CFG;
    $sql = "SELECT COUNT(ra.id)
            FROM {$CFG->prefix}role_assignments ra, {$CFG->prefix}role r
            WHERE ra.userid = :sender_id AND ra.roleid = r.id AND r.shortname = :role";

    $count = $DB->get_field_sql($sql, array('sender_id' => $sender_id, 'role' => $role));
    return ($count > 0);           
} // function message_has_role 


function message_get_connector_cm_id($course_id) {
    global $DB;
    $sql = "SELECT cm.id FROM {course_modules} AS cm, {modules} AS m
            WHERE cm.module = m.id
            AND cm.course = :course_id
            AND m.name LIKE 'connector' LIMIT 1";
    return $DB->get_field_sql($sql, array('course_id' => $course_id));
} // function message_get_connector_cm_id 
