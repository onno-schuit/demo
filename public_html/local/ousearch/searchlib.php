...
include_once('ousmodules.php');

define("OUSEARCH_FOUND", true);
...
class local_ousearch_document {
    ...
    private function internal_add_words($title,$content) {
        ...
        // MySQL collation treats letters with accents the same as letters without accents
        // We need to anticipate for that in our PHP, otherwise the search tries to write
        // the same word twice (or more) to the database.
        // Solution: words are stored in associative array; replace accented indexes with non-accented indexes 
        foreach ($dbwords as $dbword=>$dbword_contents) {
            $no_accents = self::collation_conform($dbword);
            if ($dbword != $no_accents) {
                $dbwords[$no_accents] = $dbword_contents; // set the non-accented index
                unset($dbwords[$dbword]); // remove the accented index
            }
        }
    ...
    /**
     * Returns string without accents (like utf_general_cs collation from MySQL)
     * Found at http://stackoverflow.com/questions/2758736/multibyte-strtr-mb-strtr
     * 
     * @param mixed $string
     * @return string
     */
    public static function collation_conform($string) {
    
        $result = preg_replace('~&([a-z]{1,2})(?:acute|cedil|circ|grave|lig|orn|ring|slash|th|tilde|uml);~i', '$1', htmlentities($string, ENT_QUOTES, 'UTF-8'));
    ...
    public static function split_words($text, $query=false, $positions=false) {
        ...
        if (!$positions) {
            ...
            foreach ($result as $word) {
                if ($word !== '') {
                    $words[] = self::collation_conform($word);
                }
            }
            return $words;
        } else {
            ...
            while($pos < $tl->strlen($text)) {
                ...
                $words[] = self::collation_conform($tl->substr($text, $pos, $nextunderline - $pos));
                $positions[] = $pos;
                $pos = $nextunderline + 1;
            }
...
class local_ousearch_search {
    ...
    public function set_course_ids($courseids) {
        if (!is_array($courseids)) return $this->set_course_id($courseids);
        $this->courseids = array_map('intval', $courseids);
    }
    ...
    /**
     * Replaces indexes with accents by indexes without accents 
     * 
     * @param mixed $array
     * @return void
     */
    public static function unaccent_index(&$array) {
        foreach ($array as $index=>$value) {
            $without_accents = local_ousearch_document::collation_conform($index); 
            if ($index != $without_accents) {
                $array[$without_accents] = $value; // add index without accents
                unset($array[$index]); // remove original accented index
            }
        }
    }
...
LEFT JOIN {local_ousearch_occurrences} $alias
    ON $alias.documentid=o0.documentid AND $alias.wordid=?";
        ...
        // Solin: added restriction to course ids of the current user
        if (isset($this->courseids) && is_array($this->courseids) && count($this->courseids) > 0) {
            $where .= " AND d.courseid IN (" . join(',', $this->courseids) . ")";
        }
        if (isset($_REQUEST['restrictmodule']) && in_array($_REQUEST['restrictmodule'], get_registered_ousmodules()) ) {
            $where .= "AND d.plugin=?";
            $wherearray[] = 'mod_' . $_REQUEST['restrictmodule'];            
        }
...
ORDER BY totalscore DESC, o0.documentid";
        ...
        $count_query = str_replace('o0.documentid,', 'COUNT(o0.documentid) AS totalcount,', $query);
        $this->dbresultcount = $DB->get_field_sql($count_query, $queryarray);
    ...
    private function internal_filter($results,$desired) {
        ...
        foreach ($results as $result) {
            ...
            if (substr($result->plugin,0,4) === 'mod_') {
                ...
                $login_required_function = $module . '_ousearch_login_required';
                if (!isloggedin() && $login_required_function()) continue;
            ...
            if (!$page) {
                ...
                /// Solin: Don't whipe when we're testing please!! Comment next line..
    ...
    public function display_results($baseurl, $title=null) {
        ...
        if ($results->success) {
            ...
            if (count($results->results) == self::RESULTS_PER_PAGE) {
                ...
                if ($from + self::RESULTS_PER_PAGE < $this->dbresultcount) {
                    $linknext =
        ...
        if ($linknext) {
            $linknext .= ('&restrictmodule=' . $_REQUEST['restrictmodule']); 
        }
        if ($linkprev) {
            $linkprev .= ('&restrictmodule=' . $_REQUEST['restrictmodule']);
        } 
        ...
        return self::format_results($results, $title, $from + 1, $linkprev,
            $oldrange, $linknext, $searchtime, ($this->dbresultcount - $this->removed_count));
    }
    ...
    public static function format_results($results, $title, $number=1,
        $prevlink=null, $prevrange=null, $nextlink=null, $searchtime=null, $resultcount=0) {
        ...
        if (!$results->success) {
            ...
            // disabled for THNK
            # $out .= "<p>" . get_string('resultscount', 'local_ousearch', $resultcount) . "</p>";
            if ($prevlink) {
            ...
            if (count($results->results) == 0) {
                ...
                $out .= '<form action="search.php" method="get">';
                $out .= '<select name="restrictmodule">';
                $out .= sprintf('<option value="">%s</option>', get_string('restrictmodule_none', 'local_ousearch'));
                foreach(get_registered_ousmodules() as $module) {
                    $selected = (isset($_REQUEST['restrictmodule']) && $_REQUEST['restrictmodule'] == $module);
                    $out .= sprintf('<option value="%s"%s>%s</option>', 
                        $module, 
                        $selected ? ' selected="selected"' : '', 
                        get_string('doctype_' . $module . '_pl', 'block_tl_search'));
                }
                $out .= '</select>';
                $out .= sprintf('<input type="hidden" name="query" value="%s" />', htmlentities($_REQUEST['query']));
                $out .= sprintf('<input type="hidden" name="from" value="%d" />', isset($_REQUEST['from']) ? $_REQUEST['from'] : 0);
                $out .= sprintf('<input type="hidden" name="dbstart" value="%d" />', isset($_REQUEST['dbstart']) ? $_REQUEST['dbstart'] : 0);
                $out .= sprintf('<input type="submit" value="%s" />', get_string('restrictmodule_submitbutton', 'local_ousearch'));
                $out .= '</form>';
                $out .= '<ul>';
                foreach ($results->results as $result) {
                    ...
                    $out .= '<h3><a href="' . $result->url . '">' .
                            ...
                            '</a></h3>' .
                            sprintf('<span class="date">%s</span>', strftime('%e %B %Y %H:%M', $result->timemodified)) . 
