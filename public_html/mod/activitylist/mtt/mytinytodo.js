...
var mytinytodo = window.mytinytodo = _mtt = {
	...
	init: function(options)
	{
		...
		$("#edittags").autocomplete(_mtt.mttUrl + 'ajax.php?suggestTags', {scroll: false, multiple: true, selectFirst:false, max:8, extraParams:{list:function(){ var taskId = document.getElementById('taskedit_form').id.value; return taskList[taskId].listId; }}});
		...
		$('#tasklist li').live('dblclick', function(){
            ...
            if(!$(this).closest('li').data('item').can_edit) { return; } // not permitted
		...
		$('#tasklist .taskactionbtn').live('click', function(){
            ...
            var li_context = $('#taskcontextcontainer');
            if (!li_elm.data('item').can_edit) {
                li_context.find('li').addClass('disabled');
            } else {
                li_context.find('li').removeClass('disabled');
            }
...
function loadTasks(opts)
{
	...
	opts = opts || {};
	if(opts.clearTasklist || 1) { // changed ajax code, so we need to clear the list always now
		$('#tasklist').html('');
		$('#total').html('0');
	}
	...
	_mtt.db.request('loadTasks', {
		...
		$.each(json.list, function(i,item){
            var _task = $(prepareTaskStr(item));
            _task.data('item', item); 
            _tasks.push(_task);
...
function prepareTaskStr(item, noteExp)
{
	...
	return '<li id="taskrow_'+id+'" class="' + (item.compl?'task-completed ':'') + item.dueClass + (item.note!=''?' task-has-note':'') +
		...
		'<input type="checkbox" '+(flag.readOnly||!item.can_check?'disabled="disabled"':'')+(item.compl?'checked="checked"':'')+'/></div>'+"\n"+
...
function saveTask(form)
{
	...
	_mtt.db.request('editTask', {id:form.id.value, title: form.task.value, note:form.note.value,
		...
		function(json){
            ...
            var _task = $(prepareTaskStr(item, noteExpanded));
            _task.data('item', item);
			$('#taskrow_'+item.id).replaceWith(_task);
...
function cancelTagFilter(tagId, dontLoadTasks)
{
	...
	if(dontLoadTasks==null || !dontLoadTasks) loadTasks({'clearTasklist': true});
};

function addFilterTag(tag, tagId, exclude)
{
	if(!_mtt.filter.addTag(tagId, tag, exclude)) return false;
	loadTasks({'clearTasklist': true});
};
...
function mttMenu(container, options)
{
	...
	this.showSub = function()
	{
		...
		var offset = this.$caller.position(); // offset
        // there's something wrong with the placement of the priority submenu.. needs fixing!
		var x = offset.left+this.$caller.outerWidth()+100;
		if(x + this.$container.outerWidth(true) > $(window).width() + $(document).scrollLeft()) x = offset.left - this.$container.outerWidth() - 1;
		if(x<0) x=0;
		var y = offset.top + this.parent.$container.position().top-this.parent.$container.find('li:first').position().top;
...
{
	if(!el.id || $(el).hasClass('disabled')) return;
