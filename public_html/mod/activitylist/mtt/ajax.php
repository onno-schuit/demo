...
$mdl_prefix = $CFG->prefix;

$context = mtt_get_moodle_context(); // gets Moodle context and also checks if user is logged in w/ course

if(isset($_GET['loadLists']))
{
    $sql = "SELECT l.id FROM {$mdl_prefix}activitylist l
        INNER JOIN {$mdl_prefix}course_modules cm ON (l.id=cm.instance)
        INNER JOIN {$mdl_prefix}modules m ON (cm.module=m.id AND m.name='activitylist')
        WHERE cm.id=" . (int)$_REQUEST['instanceid'];
    $instanceid = $db->sq($sql);

	if($needAuth && !is_logged()) $sqlWhere = 'WHERE published=1';
	else $sqlWhere = 'WHERE 1';

    $sqlWhere .= sprintf(" AND instanceid=%d", $instanceid); // coursemodule instance id
	$t = array();
	$t['total'] = 0;
    $sql = "SELECT * FROM {$db->prefix}lists $sqlWhere ORDER BY ow ASC, id ASC";
	$q = $db->dq($sql = "SELECT * FROM {$db->prefix}lists $sqlWhere ORDER BY ow ASC, id ASC");
...
elseif(isset($_GET['newTask']))
{
	...
	$db->ex("BEGIN");
	$db->dq("INSERT INTO {$db->prefix}todolist (uuid,list_id,userid,title,d_created,d_edited,ow,prio) VALUES (?,?,?,?,?,?,?,?)",
				array(generateUUID(), $listId, $USER->id, $title, time(), time(), $ow, $prio) );
...
function prepareTaskRow($r)
{
    ...
    $capabilites = mtt_get_capabilites($context);
    
    $can_edit = false;
    if ($capabilites['edit_all_activities']) { 
        $can_edit = true;
    } elseif ($capabilites['edit_all_group_activities']) {
        $can_edit = true; // what is a group activity?
    } elseif ($capabilites['edit_own_activity'] && ($r['userid'] == $USER->id)) {
        $can_edit = true;
    } 
	...
	return array(
        ...
        'can_edit' => $can_edit,
        'can_check' => $capabilites['check_activities'], 
	);
}
...
function mtt_get_moodle_context() {
    global $mdl_prefix, $db;
    if (isset($_REQUEST['instanceid'])) {
        $cm_id = (int)$_REQUEST['instanceid'];
    } elseif (isset($_REQUEST['list'])) {
        $sql = "SELECT cm.id FROM {$mdl_prefix}activitylist l
            INNER JOIN {$mdl_prefix}activitylist_lists al ON (l.id=al.instanceid)
            INNER JOIN {$mdl_prefix}course_modules cm ON (l.id=cm.instance)
            INNER JOIN {$mdl_prefix}modules m ON (cm.module=m.id AND m.name='activitylist')
            WHERE al.id=" . (int)$_REQUEST['list'];
        $cm_id = (int)$db->sq($sql);
    } else {
        $task_id = 0;
        foreach(explode(',', 'completeTask,editTask,deleteTask,setPrio') as $action) {
            if (isset($_REQUEST[$action])) { 
                $task_id = (int)$_REQUEST[$action];
                break;
            }
        }
        $sql = "SELECT cm.id FROM {$mdl_prefix}activitylist_todolist t 
            INNER JOIN {$mdl_prefix}activitylist_lists al ON (al.id=t.list_id)
            INNER JOIN {$mdl_prefix}activitylist l ON (al.instanceid=l.id)
            INNER JOIN {$mdl_prefix}course_modules cm ON (l.id=cm.instance)
            INNER JOIN {$mdl_prefix}modules m ON (cm.module=m.id AND m.name='activitylist')
    ...
    if (!$cm = get_coursemodule_from_id('activitylist', (int)$cm_id)) {
        die('Course module ID was incorrect: ' . $sql);
    }

    $context = get_context_instance(CONTEXT_MODULE, $cm->id);

    // also check if user's logged in for this course
    $sql = "SELECT c.* FROM {$mdl_prefix}course_modules cm
        INNER JOIN {$mdl_prefix}course c ON (cm.course=c.id)
        WHERE cm.id=" . (int)$cm_id;
    $dbresult = $db->dq($sql); // do query
    $course = (object)$dbresult->fetch_assoc();
