...
/**
 * returns array with true or false for all module capabilities
 * 
 * @return array
 */
function mtt_get_capabilites($context) {
    $result = array();
    $caps = explode(',', 'view_activities,edit_all_activities,edit_all_group_activities,edit_own_activity,check_activities');
    foreach ($caps as $cap) {
        $result[$cap] = has_capability('mod/activitylist:' . $cap, $context);
    }
    return $result;
}
