...
function forum_add_new_post($post, $mform, &$message) {
    ...
    if (defined('OUSEARCH_FOUND')) {
        if ($discussion->groupid > 0) $post->groupid = $discussion->groupid;
...
function forum_search_installedNOMORE() {
    return @include_once(dirname(__FILE__).'/../../local/ousearch/searchlib.php');
} // function forum_search_installed


/**
 * Obtains a search document relating to a particular forum post.
 *
 ...
 */
function forum_get_search_documentNOMORE($post, $cm) {
    global $DB;
    // Set up 'search document' to refer to this post
    $doc = new local_ousearch_document();
    $doc->init_module_instance('forum', $cm);
    ...
    return $doc;
} // function forum_get_search_document
 ...
 */
function forum_search_updateNOMORE($post, $cm) {
    // Do nothing if OU search is not installed
    if (!forum_search_installed()) {
        return true;
    }

    // Get search document
    $doc = forum_get_search_document($post, $cm);

    // Update information about this post (works ok for add or edit)
    $doc->update($post->subject, $post->message);
    return true;
} // function forum_search_update
...
function forum_delete_post($post, $children, $course, $cm, $forum, $skipcompletion=false) {
    ...
    // Just in case we are deleting the last post
        ...
        if (defined('OUSEARCH_FOUND')) {
            if ($discussion && $discussion->groupid > 0) $post->groupid = $discussion->groupid;
            $doc = ousforum::get_search_document($post, $cm);
            $doc->delete();
        }
...
// and the current user is a guest.
    ...
    if ($canstart) {
        ...
        echo '<input type="submit" class="btn btn-small btn-primary" value="'.$buttonadd.'" />';
        echo '</div>';
        echo '</form>';
        echo "</div>\n";
