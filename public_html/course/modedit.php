    ...
    if (!empty($fromform->update)) {
        ...
        if (plugin_supports('mod', $fromform->modulename, FEATURE_MOD_INTRO, true)) {
            ...
            if (defined('OUSEARCH_FOUND')) {
                ...
                $post->id = 0;
                $post->userid = $USER->id;
                ouscoursemodule::update($post);
                $fs = get_file_storage();
                $files = $fs->get_area_files($modcontext->id, 'mod_'.$fromform->modulename, 'content');
                foreach ($files as $f) {
                    ...
                    $search_post = (object)array(
                        ...
                        'subject' => $post->name,
                        'course' => $post->course,
                        'coursemodule' => $post->coursemodule,
                        'modulename' => $post->modulename
                    );
                    ouscoursemodule_file::update($search_post);
                }
            }
        }
