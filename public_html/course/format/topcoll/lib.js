...
function toggleexacttopic(target,image,toggleNum,reloading)  // Toggle the target tr and change the image which is the a tag within the td of the tr above target
{
    if(document.getElementById)
    {
        ...
        if ($(target).css("display") == displaySetting)
        {
	    $(target).slideUp();
            if (ie7OrLess == true)
            {
                $(target).addClass("collapsed_topic");  //add the class name
            }
        ...
        else
        {
            ...
            if (ie7OrLess == true)
            {
                $(target).removeClass("collapsed_topic"); //remove the class name
...
function togglefrontexacttopic(target,image,toggleNum,reloading)  // Toggle the target tr and change the image which is the a tag within the td of the tr above target
{
    //if(document.getElementById)
    {
