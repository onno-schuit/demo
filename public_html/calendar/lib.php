...
class calendar_event {
    ...
    /**
     * Updates the fulltext search information for an event which is being added or
     * updated.
     * @param object $event Event data, including slashes for database. Must have
     *   fields id,userid,groupid (if applicable), title, message
     * @param object $cm Course-module
     * @return True if search update was successful
     */
    function search_update($event, $cm) {
        ...
        */
        ...
        // Get search document
        $doc=oublog_get_search_document($event, $cm);


        // Update information about this event (works ok for add or edit)
        $doc->update($event->title, $event->message, null, null, $taglist);
        return true;
    } // function search_update



    function get_search_document($event, $cm) {
        global $DB;
        // Set up 'search document' to refer to this event
        $doc = new local_ousearch_document();
        $doc->init_module_instance('calendar', $cm);
        if(!isset($event->userid) || !isset($event->groupid)) {
            $results=$DB->get_record_sql("
                SELECT p.groupid, i.userid
                FROM {oublog_posts} p INNER JOIN {oublog_instances} i 
                ON p.oubloginstancesid = i.id
                WHERE p.id = ?", array($event->id));
            if(!$results) {
                print_error('invalidblogdetails','oublog');
            }
            $event->userid = $results->userid;
            $event->groupid = $results->groupid;
        }
        if($event->groupid) {
            $doc->set_group_id($event->groupid);
        }
        $doc->set_user_id($event->userid);
        $doc->set_int_refs($event->id);
        return $doc;
    } // function get_search_document 
    ...
    public function update($data, $checkcapability=true) {
        ...
        if (empty($this->properties->id) || $this->properties->id < 1) {
            ...
            // Update search index -- Onno: We don't have a course_module record
            //                     --       calendar is not a module...
            //$this->search_update($this->properties, $cm);
            if (defined('OUSEARCH_FOUND')) {
                ouscalendar::update($this->properties);
            }
